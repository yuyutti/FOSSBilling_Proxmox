...
class Admin extends \Api_Abstract
{
    ...
    */
    ...
    public function server_create($data)
    {
        ...
        // Here comes the logic that needs to happen when a server is created
    ...
    public function server_get($data)
    {
        ...
        $replacement = str_repeat("*", 26);
        $tokenvalue_clean = substr_replace($server->tokenvalue, $replacement, 0, 26);
        $output = array(
            'id'            => $server->id,
            'name'          => $server->name,
            'group'         => $server->group,
    ...
    public function get_hardware_data($server_id)
    {
        ...
        foreach ($serverstorage as $key => $value) {
            ...
            if (!empty($storage)) {
                ...
                $storage->size = $value['total'];
                $storage->free = $value['avail'];
            ...
            $storage->used = $storage->used/1000/1000/1000;
            $storage->size = $storage->size/1000/1000/1000;
            $storage->free = $storage->free/1000/1000/1000;
